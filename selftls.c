...
int main(int argc, char **argv) {
	...
	if (!(sctx = SSL_CTX_new(TLSv1_method()))) err();
	if (!SSL_CTX_use_certificate_file(sctx, "server.pem", SSL_FILETYPE_PEM)) err();
	if (!SSL_CTX_use_PrivateKey_file(sctx, "server.key", SSL_FILETYPE_PEM)) err();
	...
	if(!(cctx = SSL_CTX_new(TLSv1_method()))) err();
